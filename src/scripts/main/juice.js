...
if (window.location.pathname === "/juice/" || window.location.pathname === "/juice/index.html") {
  if (code === "XTRQFPXAAUH68GU" || code === "36XMNRCR4WMDB9P" || code === "N5ZFYUSD9NFGXEJ") {
    document.write(`<p>You've found a juice box code! Go ask Zavier (7-2) or Gage (7-2) to claim a free juice box in room 174 at lunch. Code: ${code}</p>`);
  } else {
    document.write('<p>You can buy a juice box from Gage or Zavier at lunch time in room 174</p>')
  }
}
...
console.log(simpleHash("cdst51lol"))

if (window.location.pathname === "/juice/settings-real.html") {
  if (simpleHash(password) !== "wypgiz" && sessionStorage.getItem("pass") === null) {
    history.back()
    window.close()
...
function disableCode(buttonID, OBJ) {
  if (buttonID === 1) {
    if (localStorage.getItem("code_1") === "disabled") {
      ...
      document.getElementById("code-1").innerHTML = `<strike>${validCodes[0]}</strike>`
      OBJ.value = "Enable"
    }
  } else if (buttonID === 2) {
    if (localStorage.getItem("code_2") === "disabled") {
      ...
      document.getElementById("code-2").innerHTML = `<strike>${validCodes[1]}</strike>`
      OBJ.value = "Enable"
    }
  } else if (buttonID === 3) {
    if (localStorage.getItem("code_3") === "disabled") {
      ...
      document.getElementById("code-3").innerHTML = `<strike>${validCodes[2]}</strike>`
      OBJ.value = "Enable"
    }
  }
}
